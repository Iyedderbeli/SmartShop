...
class ProductRepository(private val dao: ProductDao) {
    ...
    suspend fun addProduct(name: String, quantity: Int, price: Double, imageUri: String?) {
        require(name.isNotBlank()) { "Name cannot be empty" }
        require(price > 0) { "Price must be > 0" }
        require(quantity >= 0) { "Quantity must be ≥ 0" }

        dao.insertProduct(ProductEntity(name = name, quantity = quantity, price = price, imageUri = imageUri))
    }
